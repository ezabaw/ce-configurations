...
#    echo "Sleeping 10 seconds ..."
    ...
    if [[ "X$KP" == "X" && ! -f $MAINT ]]
      then
          if [ -f $LOCK_DIR/searchd_is_not_running_email ]
             then 
                 echo "searchd on  `hostname` is not running" | mail -r kaltura@`hostname` -s "searchd service not found on `hostname`" $MAILTO
		 rm -f $LOCK_DIR/searchd_is_not_running_email
          fi
	  touch $LOCK_DIR/searchd_is_running_email
    ...
    if [ -f $LOCK_DIR/searchd_is_running_email ]
       then
           echo "searchd on  `hostname` is running" | mail -r kaltura@`hostname` -s "searchd service is found on `hostname`" $MAILTO
	   rm -f $LOCK_DIR/searchd_is_running_email
    fi
    touch $LOCK_DIR/searchd_is_not_running_email
done
